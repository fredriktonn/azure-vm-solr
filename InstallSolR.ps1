...
{
    Write-Host "Setting JAVA_HOME environment variable"  | Out-File $LogFile -Append
...
if($solrSSL -eq $false)
{
    ...
    {
        Write-Host "Rewriting solr config"  | Out-File $LogFile -Append
...
if($solrHost -ne "localhost")
{
    ...
    {
        Write-Host "Updating host file"  | Out-File $LogFile -Append
...
if($solrSSL -eq $true)
{
    ...
    if(!($existingCert))
    {
        Write-Host "Creating & trusting an new SSL Cert for $solrHost"  | Out-File $LogFile -Append
    ...
    {
        Write-Host "Exporting cert for Solr to use"  | Out-File $LogFile -Append
...
if(!($svc))
{
    Write-Host "Installing Solr service"  | Out-File $LogFile -Append
    &"$installFolder\nssm-$nssmVersion\win64\nssm.exe" install "$solrName" "$solrRoot\bin\solr.cmd" "-f" "-p $solrPort"
    $svc = Get-Service "$solrName" -ErrorAction SilentlyContinue
}
if($svc.Status -ne "Running")
{
    Write-Host "Starting Solr service"  | Out-File $LogFile -Append
